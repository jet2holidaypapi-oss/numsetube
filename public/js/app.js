...
function updateAuthUI() {
  const isAdmin = currentUser && (currentUser.is_admin === 1 || currentUser.username === 'Papi');
  if (currentUser) {
    ...
    document.getElementById('admin-badge-nav').classList.toggle('hidden', !isAdmin);
    document.getElementById('admin-panel-link').classList.toggle('hidden', !isAdmin);
...
function createVideoCard(v) {
  ...
  const adminBadge = (v.is_admin === 1 || v.username === 'Papi') ? ' <span class="admin-tag" style="font-size:.68rem;margin-left:2px;">👑</span>' : '';
  card.innerHTML = `
...
async function loadRelatedVideos(videoId) {
  ...
  container.innerHTML = '';
  res.videos?.filter(v => v.id !== videoId).forEach(v => container.appendChild(createRelatedCard(v)));
}
...
function createCommentEl(c) {
  ...
  el.id = 'comment-' + c.id;
  const isAdminC = c.is_admin === 1 || c.username === 'Papi';
  ...
  el.innerHTML = `
    ...
    <div class="comment-content">
      <div class="comment-meta">
        ...
        ${isAdminC ? '<span class="admin-tag" style="font-size:.68rem;">👑</span>' : ''}
...
function handleDropZoneClick(e) {
  // Only trigger if the click was NOT on the "Select files" button (it handles itself)
  if (e.target.classList.contains('btn-upload-select')) return;
  document.getElementById('video-file-input').click();
}
...
function handleDrop(e) {
  ...
  if (file && file.type.startsWith('video/')) goToUploadDetails(file);
...
function goToUploadDetails(file) {
  ...
  const step2 = document.getElementById('page-upload-details');
...
async function openConversation(userId) {
  ...
  const partnerName = convoEl?.querySelector('.convo-name')?.childNodes[0]?.textContent?.trim() || 'User';
...
async function loadProfile(userId) {
  ...
  if (currentUser && !isSelf) document.getElementById('profile-subscribe-btn').textContent = 'Subscribe';
...
async function loadAdminUsers() {
  ...
  res.users.forEach(u => {
    ...
    row.className = 'admin-row';
    row.innerHTML = `
      <div class="admin-row-info"><strong>${esc(u.username)}${isAdm ? ' <span class="admin-tag">👑 Admin</span>' : ''}</strong><span>Joined ${timeAgo(u.created_at)}</span></div>
      <div class="admin-row-actions">
...
async function api(url, options = {}) {
  try {
    const res = await fetch(url, { method: options.method || 'GET', headers: options.body ? { 'Content-Type': 'application/json' } : {}, body: options.body ? JSON.stringify(options.body) : undefined });
