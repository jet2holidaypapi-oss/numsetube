...
function createVideoCard(v) {
  ...
  card.innerHTML = `
    ...
    <div class="video-card-info">
      <div class="video-card-channel">${esc(v.username)}</div>
...
async function loadWatchPage(videoId) {
  ...
  const { video, likes, userLike, subCount, isSubbed } = res;
  currentVideo = video;
...
function viewChannel() {
  if (currentVideo) navigate('profile', currentVideo.user_id);
}
...
async function loadRelatedVideos() {
  ...
  container.innerHTML = '';
  res.videos?.filter(v => v.id !== currentVideo?.id)
    .forEach(v => container.appendChild(createRelatedCard(v)));
}
...
function createCommentEl(c) {
  ...
  el.id = 'comment-' + c.id;
  const canDelete = currentUser?.id === c.user_id;
  el.innerHTML = `
...
async function submitComment() {
  ...
  if (res.comment) {
    const list = document.getElementById('comments-list');
    list.prepend(createCommentEl(res.comment));
    input.value = '';
    cancelComment();
    const countEl = document.getElementById('comments-count');
...
async function sendMessage() {
  ...
  input.value = '';
  const res = await api(`/api/messages/${currentChatPartnerId}`, { method: 'POST', body: { text } });
  if (res.success) {
...
function startMessagePolling() {
  messageInterval = setInterval(updateUnreadBadge, 30000);
}
...
async function loadProfile(userId) {
  ...
  grid.innerHTML = '';
  videos.forEach(v => grid.appendChild(createVideoCard({ ...v, username: user.username })));
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (si) si.addEventListener('keydown', e => { if (e.key === 'Enter') searchVideos(); });
});
...
document.addEventListener('click', e => {
  if (!e.target.closest('.avatar-wrap')) closeDropdowns();
});
...
function toast(msg, duration = 3000) {
  ...
  toastTimeout = setTimeout(() => { el.classList.add('hidden'); el.classList.remove('show'); }, duration);
}
